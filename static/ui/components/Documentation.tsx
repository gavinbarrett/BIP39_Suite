...
export const Documentation = () => {
	...
	return (<div className="documentation">
		...
		<div id="tutorial" className="doc-title">{"Setting up a simple wallet"}</div>
			<div className="doc">
				<div className="doc-text">
					{"To set up a simple BIP wallet, all you need to do is call the BIP32_Account constructor."}
		...
		<div id="bippath" className="doc-title">{"Deriving a BIP 44 path"}</div>
			<div className="doc">
				<div className="doc-text">
					{"To derive a BIP 44 path, you can use the generate_bip44_path method defined on the BIP32_Account class."}
		...
		<div id="masternode" className="doc-title">{"Deriving the master node"}</div>
			<div className="doc">
				<div className="doc-text">
					{"To discover the master node associated with the BIP32 account, use the generate_master_node method."}
					<CodeBox code_lines={["const x = document.getElementById('root')"]}/>
				</div>
			</div>
		<div id="masterkeys" className="doc-title">{"Deriving the master keypair"}</div>
			<div className="doc">
				<div className="doc-text">
					{"The master key pair associated with the mnemonic and master node can be derived with the gen_master_keypair method."}
		...
		<div id="childkeys" className="doc-title">{"Deriving child keys"}</div>
			<div className="doc">
				<div className="doc-text">
					{"Child keys can be derived by calling the gen_child_key method. Be sure to only use depths in the range 0-255 and indices from 0 - 2^31 - 1 for unhardened keys and 2^31 - 2^32 for hardened keys."}
		...
		<div id="addresses" className="doc-title">{"Generating addresses"}</div>
			<div className="doc">
				<div className="doc-text">
					{"Legacy Bitcoin addresses can be computed with the generate_legacy_address function on any extended public key."}
					<CodeBox code_lines={["const x = document.getElementById('root')"]}/>
				</div>
			</div>
	</div>);
